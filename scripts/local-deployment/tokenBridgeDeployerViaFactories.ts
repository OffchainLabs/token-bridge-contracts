...
import {
  ...
  L1TokenBridgeCreator__factory,
  L2TokenBridgeFactory__factory,
} from '../../build/types'
...
export const setupTokenBridge = async (
  ...
  await bridgeFundsToL2Deployer(l1Deployer, coreL2Network.ethBridge.inbox)
...
export const deployTokenBridgeAndInit = async (
  ...
  const receipt = await (await l1TokenBridgeCreator.createTokenBridge()).wait()
...
const deployTokenBridgeFactory = async (l1Signer: Signer) => {
  /// deploy factory
  console.log('Deploy L1TokenBridgeCreator')

  const l1TokenBridgeCreator = await new L1TokenBridgeCreator__factory(
    l1Signer
  ).deploy()
