...
contract IntegrationTest is Test {
    ...
    uint256 private maxGas = 100_000;
    ...
    function setUp() public {
        ...
        nativeToken =
            new ERC20PresetFixedSupply("Appchain Token", "App", 1_000_000 ether, address(this));
        inbox = ERC20Inbox(TestUtil.deployProxy(address(new ERC20Inbox(104_857))));
        ...
        defaultGateway =
            L1OrbitERC20Gateway(TestUtil.deployProxy(address(new L1OrbitERC20Gateway())));
        router = L1OrbitGatewayRouter(TestUtil.deployProxy(address(new L1OrbitGatewayRouter())));
        router.initialize(
            owner, address(defaultGateway), address(0), counterpartGateway, address(inbox)
        );
        defaultGateway.initialize(
            l2Gateway, address(router), address(inbox), cloneableProxyHash, l2BeaconProxyFactory
        );
    ...
    function test_depositToken_CustomGateway() public {
        ...
        L1OrbitCustomGateway(address(customL1Gateway)).initialize(
            makeAddr("l2Gateway"), address(router), address(inbox), owner
        );

        // create token and register it
        TestOrbitCustomTokenL1 customToken =
            new TestOrbitCustomTokenL1(address(customL1Gateway), address(router));
