...
/// This test proofs that rebalance keeps the profit in the subvault only when perf fee is enabled.
contract MasterVaultScenario11Test is MasterVaultScenarioCoreTest {
    ...
    function test_scenario11_profitRemainsInSubVault_withPerformanceFee() public {
    ...
    /// @dev Scenario: Same as above but without performance fee
    /// Without performance fee, rebalance targets totalAssets (not just principal)
    /// Expected: All assets including profit are withdrawn, sub vault is empty
    function test_scenario11_profitDoesNotRemainInSubVault_withoutPerformanceFee() public {
        vault.setTargetAllocationWad(0.5e18);
        ...
        assertEq(token.balanceOf(address(vault.subVault())), 0, "Sub vault should be empty");
    }
}
