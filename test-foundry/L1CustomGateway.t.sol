...
import { L1CustomGateway, IInbox, ITokenGateway, IERC165, IL1ArbitrumGateway } from "contracts/tokenbridge/ethereum/gateway/L1CustomGateway.sol";
...
contract L1CustomGatewayTest is L1ArbitrumExtendedGatewayTest {
    ...
    function test_calculateL2TokenAddress(address l1Token, address l2Token) public {
    ...
    function test_forceRegisterTokenToL2_revert_InvalidLength() public {
        vm.prank(owner);
    ...
    function test_registerTokenToL2(address l1Token, address l2Token) public {
        vm.deal(l1Token, 100 ether);
        ...
        assertEq(
            L1CustomGateway(address(l1Gateway)).l1ToL2Token(l1Token),
            l2Token,
            "Invalid L2 token"
        );
    }

    function test_registerTokenToL2_CustomRefund(address l1Token, address l2Token) public {
        vm.deal(l1Token, 100 ether);
    ...
    function test_registerTokenToL2_revert_NotArbEnabled() public {
        // wrong answer
        vm.mockCall(
            address(token),
            abi.encodeWithSignature("isArbitrumEnabled()"),
            abi.encode(uint8(0xdd))
        );
    ...
    function test_registerTokenToL2_revert_NoUpdateToDifferentAddress() public {
        // register token to gateway
        vm.mockCall(
            address(token),
            abi.encodeWithSignature("isArbitrumEnabled()"),
        ...
        assertEq(
            L1CustomGateway(address(l1Gateway)).l1ToL2Token(address(token)),
            initialL2TokenAddress
        );
