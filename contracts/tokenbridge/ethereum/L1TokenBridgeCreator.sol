...
contract L1TokenBridgeCreator is Ownable {
    ...
    function setTemplates(L1GatewayRouter _router, L1ERC20Gateway _standardGateway, L1CustomGateway _customGateway)
        external
        onlyOwner
    {
    ...
    function createTokenBridge() external {
        ...
        L1GatewayRouter router =
            L1GatewayRouter(address(new TransparentUpgradeableProxy(address(routerTemplate), proxyAdmin, bytes(""))));
    ...
    function initTokenBridge(
        ...
        standardGateway.initialize(l2StandardGateway, address(router), inbox, cloneableProxyHash, l2BeaconProxyFactory);
